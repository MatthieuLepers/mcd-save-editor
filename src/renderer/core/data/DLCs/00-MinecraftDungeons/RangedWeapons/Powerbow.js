export default {
  Powerbow: {
    name: 'Powerbow',
    type: 'Ranged',
    rarity: ['Common', 'Rare'],
    image: 'img/Items/RangedWeapons/PowerBow.png',
    ancientHuntRunes: ['r', 'i'],
    stats: { power: 9.96, speed: 3.41, ammo: 1.62 },
    i18n: {
      'fr-FR': 'Arc de puissance',
      'en-EN': 'Power bow',
    },
  },
  Powerbow_Unique1: {
    name: 'Powerbow_Unique1',
    type: 'Ranged',
    rarity: ['Unique'],
    image: 'img/Items/RangedWeapons/ElitePowerBow.png',
    ancientHuntRunes: ['r', 'i'],
    stats: { power: 9.96, speed: 3.41, ammo: 1.62 },
    activeEnchants: ['Power'],
    i18n: {
      'fr-FR': 'Arc de puissance d\'élite',
      'en-EN': 'Elite power bow',
    },
  },
  Powerbow_Unique2: {
    name: 'Powerbow_Unique2',
    type: 'Ranged',
    rarity: ['Unique'],
    image: 'img/Items/RangedWeapons/Sabrewing.png',
    ancientHuntRunes: ['s', 'r', 'i'],
    stats: { power: 9.96, speed: 3.41, ammo: 1.62 },
    activeEnchants: ['RadianceRanged'],
    i18n: {
      'fr-FR': 'Colibri',
      'en-EN': 'Sabrewing',
    },
  },
  PowerBow_Spooky2: {
    name: 'PowerBow_Spooky2',
    type: 'Ranged',
    rarity: ['Unique'],
    image: 'img/Items/RangedWeapons/PhantomBow.png',
    ancientHuntRunes: ['s', 'r', 'i'],
    stats: { power: 9.96, speed: 3.41, ammo: 1.62 },
    activeEnchants: ['Power'],
    i18n: {
      'fr-FR': 'Arc fantôme',
      'en-EN': 'Phantom bow',
    },
  },
};
