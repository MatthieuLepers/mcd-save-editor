export default {
  Cutlass: {
    name: 'Cutlass',
    type: 'Melee',
    rarity: ['Common', 'Rare'],
    image: 'img/Items/MeleeWeapons/Cutlass.png',
    ancientHuntRunes: ['i'],
    stats: { power: 4.26, speed: 3.49, area: 4.58 },
    i18n: {
      'fr-FR': 'Coutelas',
      'en-EN': 'Cutlass',
    },
  },
  Cutlass_Unique1: {
    name: 'Cutlass_Unique1',
    type: 'Melee',
    rarity: ['Unique'],
    image: 'img/Items/MeleeWeapons/NamelessBlade.png',
    ancientHuntRunes: ['s', 'i'],
    stats: { power: 4.26, speed: 3.49, area: 4.58 },
    i18n: {
      'fr-FR': 'Lame sans nom',
      'en-EN': 'Nameless blade',
    },
  },
  Cutlass_Unique2: {
    name: 'Cutlass_Unique2',
    type: 'Melee',
    rarity: ['Unique'],
    image: 'img/Items/MeleeWeapons/DancersSword.png',
    ancientHuntRunes: ['c', 'i'],
    stats: { power: 4.26, speed: 3.49, area: 4.58 },
    i18n: {
      'fr-FR': 'Épée de danseur',
      'en-EN': 'Dancer\'s sword',
    },
  },
  Cutlass_Year1: {
    events: ['First Year Anniversary', 'Second Year Anniversary'],
    name: 'Cutlass_Year1',
    type: 'Melee',
    rarity: ['Unique'],
    image: 'img/Items/MeleeWeapons/Sparkler.png',
    ancientHuntRunes: ['c', 'i'],
    stats: { power: 4.26, speed: 3.49, area: 4.58 },
    activeEnchants: ['Rampaging'],
    i18n: {
      'fr-FR': 'Étinceleur',
      'en-EN': 'Sparkler',
    },
  },
};
