export default {
  TempestKnife: {
    dlc: 'Howling Peaks',
    name: 'TempestKnife',
    type: 'Melee',
    rarity: ['Common', 'Rare'],
    image: 'img/Items/MeleeWeapons/TempestKnife.png',
    ancientHuntRunes: ['c'],
    stats: { power: 4.62, speed: 4.22, area: 2.26 },
    activeEnchants: ['Rushdown'],
    i18n: {
      'fr-FR': 'Couteau de tempête',
      'en-EN': 'Tempest Knife',
    },
  },
  TempestKnife_Unique1: {
    dlc: 'Howling Peaks',
    name: 'TempestKnife_Unique1',
    type: 'Melee',
    rarity: ['Unique'],
    image: 'img/Items/MeleeWeapons/ResoluteTempestKnife.png',
    ancientHuntRunes: ['c', 'i'],
    stats: { power: 4.62, speed: 4.22, area: 1.55 },
    activeEnchants: ['Committed', 'Rushdown'],
    i18n: {
      'fr-FR': 'Couteau inébranlable de tempête',
      'en-EN': 'Resolute Tempest Knife',
    },
  },
  TempestKnife_Unique2: {
    dlc: 'Howling Peaks',
    name: 'TempestKnife_Unique2',
    type: 'Melee',
    rarity: ['Unique'],
    image: 'img/Items/MeleeWeapons/ChillGaleKnife.png',
    ancientHuntRunes: ['c', 's'],
    stats: { power: 4.62, speed: 4.22, area: 2.24 },
    activeEnchants: ['Rushdown'],
    i18n: {
      'fr-FR': 'Couteau glaçant des raffales',
      'en-EN': 'Chill Gale Knife',
    },
  },
};
