export default {
  Chainsword: {
    dlc: 'Flames of the Nether',
    name: 'Chainsword',
    type: 'Melee',
    rarity: ['Common', 'Rare'],
    image: 'img/Items/MeleeWeapons/BrokenSawblade.png',
    ancientHuntRunes: ['u', 't'],
    stats: { power: 3.82, speed: 8.26, area: 1.43 },
    i18n: {
      'fr-FR': 'Lame de scie cassée',
      'en-EN': 'Broken sawblade',
    },
  },
  Chainsword_Unique1: {
    dlc: 'Flames of the Nether',
    name: 'Chainsword_Unique1',
    type: 'Melee',
    rarity: ['Unique'],
    image: 'img/Items/MeleeWeapons/MechanizedSawblade.png',
    ancientHuntRunes: ['c', 't'],
    stats: { power: 3.82, speed: 8.26, area: 1.43 },
    i18n: {
      'fr-FR': 'Lame de scie mécanique',
      'en-EN': 'Mechanized sawblade',
    },
  },
};
