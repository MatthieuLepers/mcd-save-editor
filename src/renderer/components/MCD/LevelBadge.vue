  ...
  >
    <svg version="1.1" width="70px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 121.5 140.1" xml:space="preserve">
      ...
      <path
        ...
        :stroke-dasharray="`${354 * State.percent} 354`"
        stroke-dashoffset="0"
...
defineOptions({ name: 'MCDLevelBadge' });

const { t } = useI18n();

const State = computed(() => {
  const level = CharacterUtils.xp2Level(globalStore.state.selectedCharacter.data.xp);
  const percent = (globalStore.state.selectedCharacter.data.xp - CharacterUtils.level2Xp(level)) / CharacterUtils.getXpForLevel(level + 1);
...
const actions = {
  handleMouseOver() {
    window.onmousewheel = (e) => {
      ...
      globalStore.state.selectedCharacter.data.xp += CharacterUtils.getXpForLevel(State.value.level) * (direction / 10);
    };
  },
  handleMouseOut() {
